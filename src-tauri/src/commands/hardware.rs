...
pub async fn toggle_lock(wda: tauri::State<'_, Arc<WdaClient>>) -> AppResult<()> {
    ...
    let res = wda.get_client().post(format!("{}/session/{}/wda/lock", base_url, sid.as_str())).send().await;
    let mut success = false;
    if let Ok(r) = res {
        if r.status().is_success() { success = true; }
    }
    if !success {
        wda.get_client().post(format!("{}/session/{}/wda/pressButton", base_url, sid.as_str()))
...
#[tauri::command]
pub async fn press_mute_button(wda: tauri::State<'_, Arc<WdaClient>>) -> AppResult<()> {
    let sid = wda.get_session_id().await;
    let base_url = wda.format_url("");
    for key_name in &["muteswitch", "mute", "muteswitchtoggle"] {
        let res = wda.get_client().post(format!("{}/session/{}/wda/pressButton", base_url, sid.as_str()))
