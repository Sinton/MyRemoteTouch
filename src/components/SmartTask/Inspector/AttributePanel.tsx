...
import { TreeNode, useSmartTaskStore } from '../../../store/useSmartTaskStore';
  ...
  return (
    <div className="flex flex-col relative pb-6">
      ...
      <div className="px-4 mt-5">
        <button 
          onClick={() => {
            const { addStep, setActiveTab } = useSmartTaskStore.getState();
            // Helper function logic inline or imported
            const generateId = () => `step-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 5)}`;
            
            // Generate a smart default based on node's attributes
            const labelStr = selectedNode.attributes.label || selectedNode.attributes.name || selectedNode.attributes.value;
            const fallbackSelector = labelStr 
              ? { type: 'wda_label' as const, value: labelStr }
              : { type: 'wda_predicate' as const, value: `type == '${selectedNode.type}'` };
              
            // --- Precision Physical Anchor Calculation ---
            const centerX = selectedNode.rect.x + selectedNode.rect.width / 2;
            const centerY = selectedNode.rect.y + selectedNode.rect.height / 2;
            const offsetX = deviceW > 0 ? centerX / deviceW : 0;
            const offsetY = deviceH > 0 ? centerY / deviceH : 0;
              
            addStep({
              id: generateId(),
              name: labelStr ? `操作「${labelStr}」` : `操作 ${selectedNode.type.replace('XCUIElementType', '')}`,
              selector: fallbackSelector,
              action: { type: 'tap', offset_x: offsetX, offset_y: offsetY },
              pre_delay_ms: 0,
              post_delay_ms: 800,
              timeout_ms: 10000,
          ...
          className="w-full h-8 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-bold shadow-md shadow-blue-500/20 active:scale-95 transition-all flex items-center justify-center gap-2"
        >
