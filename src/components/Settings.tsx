...
const Settings: React.FC<SettingsProps> = ({ visible, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Header - Fixed at top - Ultra-compressed */}
      <div className="flex justify-between items-center px-5 py-2.5 border-b border-white/10 bg-white/5 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <div className="w-[2.5px] h-3.5 bg-[#0A84FF] rounded-full shadow-[0_0_10px_rgba(10,132,255,0.4)]" />
          <h3 className="m-0 text-[14px] font-black tracking-tight bg-gradient-to-br from-white via-white to-white/40 bg-clip-text text-transparent">系统配置</h3>
        </div>
        <button
          className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/[0.05] border border-white/[0.05] text-white/40 hover:text-white hover:bg-white/[0.1] active:scale-90 transition-all cursor-pointer"
          onClick={onClose}
        >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 pt-6 pb-10 custom-scrollbar-wrapper">
        <div className="drawer-content space-y-10">
